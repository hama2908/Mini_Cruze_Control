...
//
#include "FIR_coefficient.h"
#include "stdio.h"
#include "math.h"
#include "fftc.h"
#include <stdlib.h>

static int32c inFFT[FFT_LEN], outFFT[FFT_LEN], twiddles[FFT_LEN / 2];
// *****************************************************************************
// *****************************************************************************
...
void Filtrage_passebas(){
   
    int32c Scratch[FFT_LEN];
    int k, log2N = LOG2FFTLEN;
    
    int32c H_passebas[FFT_LEN];
// *** POINT B1 : INSERT CODE HERE (PIC32 DSP Library FFT function 
                //inFFT ÉGAL À BUFFER ?
                mips_fft32(outFFT, inFFT, fftc, Scratch, log2N);        
                // FIR Filtering: calculate Y = HX, actually Y* in preparation for inverse FFT using forward FFT library function call
                for (k = 0; k < FFT_LEN; k++) {
                // *** POINT B2 : INSERT CODE HERE...                 
                inFFT[k].re = outFFT[k].re * H_passebas[k].re + outFFT[k].im * H_passebas[k].im;
                inFFT[k].im =  (outFFT[k].re * H_passebas[k].im + outFFT[k].im * H_passebas[k].re) * -1; 
                }
                // Inverse FFT by forward FFT: FFT(Y*)                 
                // *** POINT B3 : INSERT CODE HERE (PIC32 DSP Library FFT function call)...
                mips_fft32(outFFT, inFFT, fftc, Scratch, log2N);  
}
void Filtrage_moyennemobile(){
...
// *** POINT B1 : INSERT CODE HERE (PIC32 DSP Library FFT function 
                ...
                for (k = 0; k < FFT_LEN; k++) {
                // *** POINT B2 : INSERT CODE HERE...                 
                inFFT[k].re = outFFT[k].re * H_moyennemobile[k].re + outFFT[k].im * H_moyennemobile[k].im;
                inFFT[k].im =  (outFFT[k].re * H_moyennemobile[k].im + outFFT[k].im * H_moyennemobile[k].re) * -1; 
                }
                // Inverse FFT by forward FFT: FFT(Y*)                 
                // *** POINT B3 : INSERT CODE HERE (PIC32 DSP Library FFT function call)...
                mips_fft32(outFFT, inFFT, fftc, Scratch, log2N);  
