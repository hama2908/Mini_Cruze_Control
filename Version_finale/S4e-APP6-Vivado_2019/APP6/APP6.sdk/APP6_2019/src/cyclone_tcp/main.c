...
void task_vitesse(){
	if(compteur_vitesse++ > DELAIS_VITESSE){
		...
		//si seulement le btn de droite (btn[0]) est appuyé, on augmente la variable de vitesse si elle n'est pas deja au max
		if(btn_data == 1){
		...
		//si seulement le btn de gauche (btn[3]) est appuyé, on diminue la variable de vitesse si elle n'est pas deja a 0
		if(btn_data == 8){
			if(vitesse == 0){}
			else{
				vitesse--;
